...
class _StoryPlayScreenState extends State<StoryPlayScreen>
  ...
  // Firebase gesture detection for checkpoints
  ...
  static const double _checkpointConfidenceThreshold = 0.65;

  // Gesture hold tracking for checkpoints — require gesture to be held briefly
  String? _heldGestureLetter;
  int _gestureHoldCount = 0;
  static const int _requiredHoldCount = 2;
  ...
  void initState() {
    ...
    // Start Firebase gesture listener
  ...
  void _onGestureData(Map<String, dynamic>? data) {
    ...
    // Guard: skip gesture processing during scene transitions
    if (_fadeToBlack) return;

    final prediction = (data['prediction'] as String?)?.toUpperCase();
    final confidence = (data['confidence'] as num?)?.toDouble();
    final crossValidated = data['crossValidated'] == true;

    debugPrint('[StoryPlay] Gesture: prediction=$prediction, confidence=$confidence, crossValidated=$crossValidated');

    setState(() {
      _detectedGesture = prediction;
      _detectedConfidence = confidence;
    });

    // When cross-validated, lower confidence requirement
    final effectiveThreshold = crossValidated ? 0.45 : _checkpointConfidenceThreshold;

    // Auto-select option during checkpoint if gesture matches with hold
    if (_showCheckpoint && !_optionLocked && _currentScene.checkpoint != null && prediction != null) {
      for (final option in _currentScene.checkpoint!.options) {
        if (option.letter.toUpperCase() == prediction &&
            (confidence ?? 0) >= effectiveThreshold) {
          // Track gesture hold count
          if (_heldGestureLetter == prediction) {
            _gestureHoldCount++;
          } else {
            _heldGestureLetter = prediction;
            _gestureHoldCount = 1;
          }

          debugPrint('[StoryPlay] Checkpoint MATCH: "${option.letter}" hold=$_gestureHoldCount/$_requiredHoldCount threshold=$effectiveThreshold');

          if (_gestureHoldCount >= _requiredHoldCount) {
            debugPrint('[StoryPlay] ✅ AUTO-SELECT option "${option.letter}" → scene ${option.nextSceneIndex}');
            _selectOption(option);
            _heldGestureLetter = null;
            _gestureHoldCount = 0;
          }
          return;
        }
      }
      // No matching option — reset hold counter
      _heldGestureLetter = null;
      _gestureHoldCount = 0;
    }
  }
    ...
    return AnimatedOpacity(
      ...
      child: Column(
        children: [
          ...
          if (_detectedGesture != null && !_optionLocked)
            Container(
              ...
              decoration: BoxDecoration(
                color: AppTheme.ppPrimary.withOpacity(0.1),
                borderRadius: BorderRadius.circular(100),
                border: Border.all(color: AppTheme.ppPrimary.withOpacity(0.3)),
              ),
              child: Row(
                mainAxisSize: MainAxisSize.min,
                children: [
                  Container(
                    ...
                    decoration: BoxDecoration(
                      ...
                      shape: BoxShape.circle,
                      boxShadow: [BoxShadow(color: AppTheme.success.withOpacity(0.5), blurRadius: 4)],
                    ),
                  ),
                  const SizedBox(width: 6),
                  Text(
                    'Glove: $_detectedGesture (${((_detectedConfidence ?? 0) * 100).toStringAsFixed(0)}%)',
                    style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: AppTheme.ppPrimary),
                  ),
                ],
              ),
            ),
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: const Text('Audio unavailable', style: TextStyle(color: Colors.white, fontSize: 11)),
